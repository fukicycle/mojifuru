...
│   │   ├── rareWords.ts           # レア単語(濁点・半濁点入り)の判定
...
│   │   ├── wordCollection.ts      # 単語ずかん(はじめて作ったことばの記録)
...
| `/collection` | 単語ずかん(自分がこれまでに作ったことばの記録) |
...
/wordCollection/{uid}/{word}: <はじめて作った日時>
...
- **ずかん**(`/collection`)は、これまでに成立させた単語がたまっていくだけの記録。期限も義務もなく減りもしないので、ひさしぶりに開いても損をしない(ログインボーナス的な「毎日やらないと途切れる」仕組みは、気軽さを損なうため不採用)
- 保存先は `/wordCollection/{uid}`。なまえ・音設定のような端末ごとの設定(localStorage)とは扱いを変え、**RTDBのuid配下に置く**。Google連携してもuidは変わらないため、連携さえしていれば機種を変えても引き継がれる
- 記録するのは**ソロは結果画面(`ResultScreen`)、対戦は `GameScreen` の `handleFinish`**。ソロのプレイ中に書かないのは、「ひとりで遊ぶだけではユーザーを作らない」方針を守るため(結果画面ではランキング送信で結局サインインする)
- 記録に失敗しても結果表示は止めない(戦績の保存と同じく、あくまで付帯機能)
- ずかんは読みの一覧なので、漢字表記は `wordlist.json`(単語一覧と同じデータ・ワーカー経由)を引いて添える。「ぜんぶで◯語」もこのデータの読み数から出す
- 収録語数が多く達成率はいつまでも1%に満たないため、前面に出すのは**あつめた語数**。割合は0.1%未満のあいだ表示しない(「0.0%」は集めた実感を削ぐ)
- **レア単語**は濁点・半濁点をふくむ単語(`src/game/rareWords.ts`)。濁音・半濁音は `DEFAULT_KANA_WEIGHTS` 上の出現頻度が桁違いに低く、狙って作りにくいことが根拠
  - 判定はNFD正規化して結合濁点(U+3099/U+309A)の有無を見る(濁音の一覧を持たずに「ゔ」まで拾える)
  - **得点には影響させない**。演出だけを足す(専用の効果音 `confirmRare` はボーナス段階の音に重ねて鳴らす。配色はボーナスの金〜アンバーと取り合わないよう、チップのアクア〜マゼンタにする)
...
- `wordCollection/{uid}`:読み書きとも本人のみ。**いちど書いた単語は上書きさせない**(`.write` に `!data.exists()`)ことで「はじめて見つけた日」を残す。値は `now` のみ許可。このため書き込み側(`recordFoundWords`)は、書く前に該当単語の有無を確かめてから未収集のぶんだけ送る
...
- **ランキングで抜かれたときのプッシュ通知** → 他人のスコア更新をきっかけに配信するサーバー側の処理(RTDBのフック+FCM送信)が要るため、「サーバーサイドロジックなし」の制約と両立しない。iOSのPWAもWeb Push自体は対応したが送信側は同じ壁。代わりに「開いたときに気づかせる」プル型(ずかんの増加など)で補う
- **連続ログイン(ストリーク)・ログインボーナス** → 「毎日やらないと途切れる」縛りが、気軽に遊べるというウリと噛み合わないため不採用。減りも途切れもしない蓄積型(単語ずかん)で代替する
- **テーマ縛りモード**(動物・食べ物などの意味カテゴリ限定) → 現時点では見送り。判定自体は通常のDAWG判定とテーマ辞書の論理積で済むが、`buildDict.ts` はMozcから品詞しか見ておらず意味カテゴリのタグを残していない(JMdictの `field` タグも取り込んでいない)ため、実現には辞書パイプラインの改修と手作業のキュレーションが要る
